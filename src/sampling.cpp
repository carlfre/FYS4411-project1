...
void VMCWalker::importance_sampling_with_interactions(int k){
    vec qf = quantum_force_interaction(position, relative_position, alpha, beta, hard_core_radius, k);
    ...
    vec qf_new = quantum_force_interaction(new_position, relative_position_new, alpha, beta, hard_core_radius, k);
    // vec qf_new = quantum_force(new_position, k, alpha);
    double greens = greens_ratio(qf, qf_new, position, new_position, time_step, D, k);
    double ratio = greens*probability_ratio_interaction(position, new_position, relative_position, relative_position_new, alpha, beta, hard_core_radius, k);
    //cout << ratio << endl;
...
void VMCWalker::brute_force_sampling_with_interactions(int k)
{
    ...
    double ratio = probability_ratio_interaction(position, new_position, relative_position, relative_position_new, alpha, beta, hard_core_radius, k);
    // cout << ratio << endl;
    double random_number = rng_double(generator);
